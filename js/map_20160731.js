    ...
    setTimeout(function(){
      ...
      for (var i = 0; i < data.length; i++) {
        ...
        if($('#txtLevel').val()==4){
          ...
          circleTitle.push(Number(data[i].prov));
          // console.log(data[i].name_string);
          // console.log(ns);
          ...
          if($('#txtLevel').val()==1){
            $url_infocontent = 'core/placeDetail_province.php?place_id=' + data[i].prov ;
          }else if($('#txtLevel').val()==2){
            $url_infocontent = 'core/placeDetail_distrinct.php?place_id=' + data[i].prov + '&dist_id=' + data[i].dist;
          }else if($('#txtLevel').val()==3){
            $url_infocontent = 'core/placeDetail_tabmon.php?place_id=' + data[i].prov_id + '&dist_id=' + data[i].dist_id + '&tumbon_id=' + data[i].tumbon_id;
          }else if($('#txtLevel').val()==4){
            $url_infocontent = 'core/placeDetail_place.php?place_id=' + data[i].id;
          }

          infowindow[i] = new google.maps.InfoWindow({
            content:$.ajax({
                method: "POST",
                url: $url_infocontent,//ใช้ ajax ใน jQuery ดึงข้อมูลจากไฟล์ placeDetail.php มาแสดง
                data: {
          ...
          (function(circle, i) {
              ...
              google.maps.event.addListener(circle[i], 'click', function(evt) {
                ...
                infowindow[i].setPosition(evt.latLng);

                infowindow[i].open(map, circle[i]);
                infowindowTmp = i;
